...
- **Y-axis bounds** — fixed 0–7,000 in deficit mode; fixed ceiling of 3,000 in balance/refeed mode (lines above it clip off the top, by design — see 9 Sep 2026 changelog); data-derived in surplus mode. Height is a fixed 160px via `.burndown-wrap`, not responsive.
...
- **"SPARE TODAY" row** — smaller sub-row beneath predicted; the vertical gap between the actual and target lines at today's point on the chart. Cumulative (carries carryover and earlier days), not today in isolation. Positive = headroom to eat, in both modes. Graded on a per-day band (`tolerance / inPhaseDays`), not the weekly tolerance. Hidden when today predates the phase start. Sourced from `weekCumulativeSeries()`, shared with the chart.
...
| Date | Change | File(s) |
|------|--------|---------|
| 9 Sep 2026 | **Refeed burndown y-axis capped at 3,000** — refeeds derived `yMax` from the data, which let the *planned* line set the scale. In a refeed that line is the least interesting one: the meal items sit ~720/day under maintenance by design, to be topped up with tracked additions, so it climbed to ~4,982 by Sunday and pushed the ceiling to 6,000. The actual-vs-target gap — the quantity "Spare today" reports — was squashed into the bottom sixth of a 160px chart. Now a fixed 3,000 for balance mode, cutting the span from 7,000 to 4,000 and nearly doubling the resolution without spending any height. Deliberate trade: lines above 3,000 exit through the top of the plot area (Chart.js clips there, so they cannot draw over the card), so a week running well under its refeed target loses the tail of its projection — with the current week the planned line clips from Friday and the actual line from Saturday, hiding the 3,897 endpoint. Accepted because the predicted figure is always readable in the row beneath. Had to branch on `balance` explicitly rather than flipping the existing else, since `phaseMode()` lumps surplus in with balance and a surplus phase has no equivalent fixed scale to assume — surplus and deficit axes are unchanged, verified by forcing `tracking_mode` all three ways. `yMin` deliberately left alone at −1,000 despite `dataMin` being 0. The emoji clamp at `Math.min(yVal, yMax)` needed no change and now pins icons to the new ceiling. | `index.html`, `sw.js` |
| 8 Sep 2026 | **Fixed silent entry loss from fused log lines** — logging a peperami after a protein bar produced one fused line (`...~166 kcal2026-09-08 | cheat | Peperami...`) and the second entry's 100 kcal vanished from the week's totals with no error. Root cause was two different concatenation rules for the same file: `_drainAppendQueue()` wrote `current + newLine + '\n'` (correct, assumes a terminated file) while all four callers separately did `logText += '\n' + line` on the in-memory copy. That second rule is wrong in both directions — it adds a *second* newline to an already-terminated file (source of the stray blank lines dotted through the log since July) and leaves the in-memory copy *unterminated*. `ghWriteLog()`, used by the edit and delete paths, then rebuilt the file from that unterminated copy and persisted it, stripping the real trailing newline; the next append had nothing to separate against and fused. Three-part fix: `_drainAppendQueue()` inserts a separator if the file doesn't end with a newline, and now assigns `logText` to the exact string it wrote, so memory and remote stay byte-identical; the four caller-side `logText +=` lines are deleted; `ghWriteLog()` normalises to exactly one trailing newline via `replace(/\n*$/, '\n')` so neither caller has to remember. Repaired the 8 Sep data by hand. Pre-existing cosmetic blank lines left in place — the parser skips them and no data was lost. | `index.html`, `sw.js`, `logs/phone-log.md` |
| 8 Sep 2026 | **"Spare today" row on the weekly energy card** — answers "can I afford an extra coffee" directly, instead of leaving it to be eyeballed off the burndown. Defined as the vertical gap between the actual and target lines at today's point: `actual[todayIdx] - target[todayIdx]`, where `todayIdx = min(daysElapsed, 7) - 1`. Deliberately **cumulative** — it carries the phase carryover and earlier days' over/undershoot — so a day that ran over is still owed back rather than forgiven at midnight. Positive means headroom to eat in *both* modes: in a cut, sitting above target is banked deficit; in a refeed it means having under-eaten against plan, which is spend-it permission just the same. So unlike the carryover row this is **not** run through `energyColor` — it is graded as headroom (green ≥ 0, amber within one day's band, red beyond) on `tolerance / inPhaseDays`, because the question is about one day's latitude, not the week's. Hidden entirely when today predates the phase start. Implementation extracted the chart's cumulation into a shared `weekCumulativeSeries()` used by both the card and `renderWeeklyBurndownChart()`; computing it twice would have let the figure drift from the gap it describes. Verified against the live log: carryover 0, Mon +125, Tue +255. | `index.html`, `sw.js` |
| 1 Sep 2026 | **Carryover exposed and its colour de-biased** — the carryover silently offset the burndown's actual line from its very first point (`cumActual = plannedData[i] + cumOutlier + carryover`) and was inside the `predicted` headline, but the row rendered only when non-zero, so the week could appear to start away from zero for no visible reason. Now: `computeWeeklyDeficit()` returns `scoredWeeks`, and the row shows from the first scored week onward — including at exactly zero, rendered muted, because a real zero means "the weeks so far balanced" while a missing row looks like the mechanism doesn't exist. Colour was hardcoded `carryover > 0 ? green : red`, true only for a cut: positive carryover means having banked *deficit*, which in a refeed means having under-eaten, so a balance phase would have shown green above a red predicted figure. Now `energyColor(carryover, 0, mode, tolerance)`, sharing the card's pro-rated band. Chart gains a faint amber dashed line at the carryover level marking the actual line's origin, drawn only when non-zero. Label corrected from "Carryover from last week" to "Carryover to date" — it has been phase-cumulative since 25 May 2026 — with a week count once past one. Also dropped `carryover_start_date` from the refeed phase: it had been set to the first full Mon–Sun week, which discarded the 29–30 Aug part week (worth −90 kcal) even though the carryover maths already pro-rates part weeks correctly, and contradicted the Phase 1 precedent of moving the date *earlier* to capture day one. Absent the field the code falls back to `start_date`, so it cannot drift if a phase is re-dated. | `index.html`, `sw.js`, `logs/dashboard.json` |
